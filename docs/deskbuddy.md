...
| `deskbuddy/firmware/deskbuddy.ino` | ESP32 sketch — I2S mic streaming + OLED eyes |
| `deskbuddy/firmware/secrets.h.example` | Wi-Fi + broker config template (`secrets.h` is gitignored) |
...
**analog** input, *not* I2S) → speaker; amp VCC→5V, GND→GND. PAM8403 output is
...
### 1. Broker (in proot Ubuntu — *not* Termux)
...
1. Arduino IDE → install **ESP32** board support, board = *ESP32 Dev Module*.
2. Library Manager → install **WebSockets** (links2004), **Adafruit SSD1306**,
   **Adafruit GFX**.
3. `cp firmware/secrets.h.example firmware/secrets.h` and fill in Wi-Fi + the
   phone's LAN IP + `BROKER_PORT` 2125.
...
| `GROQ_API_KEY` | *(shared)* | Same key as standup |
...
| `BUDDY_SYSTEM_PROMPT` | *(terse persona)* | LLM system message |
| `BUDDY_PLAYER` | *(auto)* | Override play command; `{file}` = wav path |
...
| Symptom | Cause / fix |
| --- | --- |
| `wake word DISABLED` on boot | `openwakeword` not importable in proot — `pip install openwakeword`; if `onnxruntime` wheels fail, `pip install tflite-runtime` |
