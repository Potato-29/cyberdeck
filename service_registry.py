...
def service_meta_list(registry: list[ServiceDef] | None = None) -> list[dict]:
    """Ordered [{key, label, alert}] for the dashboard's service_meta field."""
    reg = registry if registry is not None else load_registry()
    return [{"key": s.key, "label": s.label, "alert": s.alert} for s in reg]


def set_alert(key: str, enabled: bool, path: Path = REGISTRY_PATH) -> bool:
    """Flip a service's alert flag in services.json. Returns False if the key
    doesn't exist. Rewrites the whole file (loses hand-authored blank-line
    grouping between entries, keeps every field including _note)."""
    import json
...
if __name__ == "__main__":
    ...
    elif len(sys.argv) > 2 and sys.argv[1] == "--set-alert":
        svc_key, flag = sys.argv[2], sys.argv[3] if len(sys.argv) > 3 else None
        if flag not in ("on", "off"):
            print("Usage: service_registry.py --set-alert <key> on|off", file=sys.stderr)
            sys.exit(1)
        if set_alert(svc_key, flag == "on"):
            print(f"{svc_key}: alert {'enabled' if flag == 'on' else 'disabled'}")
        else:
            print(f"Unknown service: {svc_key}", file=sys.stderr)
            sys.exit(1)

    else:
        print(f"Usage: {sys.argv[0]} --down | --ensure-all | --status | --set-alert <key> on|off", file=sys.stderr)
        sys.exit(1)
